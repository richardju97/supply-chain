...
//  	listOfTrucks[id-1]->shipment = shipment;
	...
	//probably need to malloc listOfTrucks[id-1]->shipment?
	
// 	boundary condition - no such truck with this ID exists
	if (id > totalTrucks)
		return -1;
...
// need boundary condition of truck is not available?
	...
	while (shipment[i] != NULL) {
		...
		listOfTrucks[id-1]->shipment[i]->name = strdup(shipment[i]->name);
...
// 	since ID's are autogenerated, we could probably make this more efficient by binary searching through the list of trucks
